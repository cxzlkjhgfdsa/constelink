...
const FundRegister: React.FC = () => {
  ...
  async function sendTransactionStartFunding(
    ...
    if (web3) {
      ...
      const receipt: TransactionReceipt = await web3.eth.sendSignedTransaction(
  ...
  const handleGoal = (e: any) => {
    ...
    // console.log(goal);
    setCheckGoal(
      `${goale.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")} 원`
    );
  ...
  // 요청 보내도 되는지 검사
  const checkValidity = () => {
    if (!image || !benId || !cate || !title || !goal || !content || !endTime) {
      setNoValErr(true);
    ...
    if (!imgErr && !titleErr && !goalErr && !noValErr) {
      console.log("보내자");
      getImgUrI();
    }
  };
    ...
    <>
      <div className={styles.mainWrapper}>
        ...
        <div className={styles.benWrapper}>
          <div className={styles.inputTitle}>수혜자</div>
        ...
        <div className={styles.imgWrapper}>
          {imgPreUrl ? (
            ...
            <label htmlFor="thumbImg">
              <div className={styles.imgThumb}>
                <div className={styles.thumbText}>썸네일을 등록해 주세요.</div>
          ...
          {imgErr && <div className={styles.errMsg}>{imgErrMsg}</div>}
        </div>
        {/* 카테고리 선택 */}
        ...
        <div className={styles.titleWrapper}>
          ...
          {/* 제목 에러 메시지 */}
          {titleErr && <div className={styles.errMsg}>{titleErrMsg}</div>}
        </div>
        {/* 모금액 입력 */}
        <div className={styles.fundWrapper}>
          <div className={styles.inputTitle}>모금액</div>
          <div className={styles.fundInput}>
            <input
              ...
              onChange={handleGoal}
            />
            {goalErr && <div className={styles.errMsg}>{goalErrMsg}</div>}
          </div>
          {/* 모금액 확인 */}
          <div className={styles.fundCheckInput}>
            {checkGoal ? (
              ...
              <div className={styles.fundCheckPlaceholder}>
                모금액을 확인해 주세요
              </div>
            )}
          </div>
        ...
        <div className={styles.storyWrapper}>
          <div className={styles.storyInputTitle}>사연</div>
          <SunEditor
        ...
        <div className={styles.timeWrapper}>
          <div className={styles.timeInputTitle}>모금 종료시간</div>
          {/* 달력 */}
          <DatePicker
        ...
        {isLoading ? (
          ...
          <div className={styles.btnsWrapper}>
            ...
            <div className={styles.btnRegister} onClick={checkValidity}>
              등록하기
            </div>
          </div>
        )}
